    ...
    public partial class FrmStatistics : Form
    {
        ...
        private void FrmStatistics_Load(object sender, EventArgs e)
        {
           ...
           lblRomeGuideName.Text=db.Guide.Where(x=>x.GuideId==romeGuideId).Select(y=>y.GuideName+" "+y.GuideSurname).FirstOrDefault().ToString();
            ...
            lblMaxPriceLocaiton.Text = db.Location.Where(x => x.Price == maxPrice).Select(y => y.City).FirstOrDefault().ToString();

            var guideıdByNameAysegulcinar=db.Guide.Where(x=>x.GuideName=="AyşeGül"&& x.GuideSurname=="Çınar").Select(y=>y.GuideId).FirstOrDefault();
            lblAysegulCinarLocationCount.Text=db.Location.Where(x => x.GuideId == guideıdByNameAysegulcinar).Count().ToString();
        }
    }
}
